...
{
    ////NX_MYTAG_DEBUG(QString("customization %1 , protoVersion %2, softwareVersion %3, purpose %4").arg(customization).arg(protoVersion).arg(version.toString()).arg((int)purpose));
    ensureInitialized();
...
{
    ////NX_MYTAG_DEBUG(QString("remoteCustomization %2").arg(remoteCustomization));
    ensureInitialized();
    if (s_developerFlags.testFlag(DeveloperFlag::ignoreCustomization))
        return true;

    ////NX_MYTAG_DEBUG(QString("nx::branding::customization() %1").arg(nx::branding::customization()));
