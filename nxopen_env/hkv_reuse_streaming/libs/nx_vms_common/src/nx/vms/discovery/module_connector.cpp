...
{
    NX_MYTAG_DEBUG(QString("start with endpoint : %1").arg(endpoint.toString().c_str()));
    const auto handler =
...
void ModuleConnector::InformationReader::readUntilError()
{
    while (const auto object = takeJsonObject(&m_buffer))
    {
        ...
        NX_MYTAG_DEBUG(QJsonDocument(restResult.reply.toObject()).toJson(QJsonDocument::Compact));
