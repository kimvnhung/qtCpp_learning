    ...
    const quint8* bufEnd)
{
    ...
    //NX_MYTAG_DEBUG(QString("finder result : %1").arg(data.constData()));
    ...
    //NX_MYTAG_DEBUG(QString("writeCustomization %1").arg(customization));
    ...
    const quint8* bufferEnd)
{
    ...
    //NX_MYTAG_DEBUG(QString("result %1").arg(QString(result)));
...
void DeprecatedMulticastFinder::run()
{
    ...
    while (!needToStop())
    {
        ...
        //NX_MYTAG_DEBUG(QString("pollset size %1").arg(m_pollSet.size()));
